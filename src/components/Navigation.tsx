...
>
  HeadStart
          ...
          <div className="hidden md:flex items-center space-x-2">
            ...
            <a href="#meet-the-authors" className="text-sm font-semibold text-white tracking-wide bg-transparent border-none p-0 m-0 cursor-pointer hover:text-orange-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-300 transition-colors flex items-center justify-center h-10 px-6 rounded-md" style={{fontFamily: 'Montserrat, Arial, sans-serif', letterSpacing: '0.01em', textShadow: '0 4px 24px rgba(0,0,0,0.25), 0 1px 8px rgba(255,255,255,0.12)', minWidth: 'auto'}} onClick={e => { e.preventDefault(); const el = document.getElementById('meet-the-authors'); if (el) { const y = el.getBoundingClientRect().top + window.pageYOffset - 38; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Meet Authors</a>
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-[#24A89C]/95 py-4 space-y-4">
            ...
            <a href="#meet-the-authors" className="block text-white/90 hover:text-white transition-colors" onClick={e => { e.preventDefault(); setIsMenuOpen(false); const el = document.getElementById('meet-the-authors'); if (el) { const y = el.getBoundingClientRect().top + window.pageYOffset - 38; window.scrollTo({ top: y, behavior: 'smooth' }); } }}>Meet Authors</a>
